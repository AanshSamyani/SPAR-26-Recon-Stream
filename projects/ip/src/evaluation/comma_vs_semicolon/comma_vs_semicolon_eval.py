"""Comma vs semicolon logit extraction eval.

For each prompt, generates a single token with output_logits=True and
extracts the logits (and softmax probabilities) for the comma (",") and
semicolon (";") tokens.
"""
...
def run_comma_vs_semicolon_eval(
    model,
    tokenizer,
    rollout_path: str,
    output_path: str,
    ...
    """Extract comma and semicolon logits for each prompt.

    Uses an already-loaded model to generate 1 token with output_logits=True
    and saves the raw logits and softmax probabilities for "," and ";" tokens.
    """
    # Get token IDs for comma and semicolon
    ...
    logger.info(
        "Using comma_id=%d ('%s'), semicolon_id=%d ('%s')",
        comma_id,
    ...
    for entry in rollouts:
        ...
        logger.info(
            "Prompt %d: comma_logit=%.4f (prob=%.6f), semicolon_logit=%.4f (prob=%.6f)",
            prompt_idx,
            comma_logit,
            comma_prob,
            semicolon_logit,
    ...
    comma_preferred = sum(
        1 for c, s in zip(all_comma_logits, all_semicolon_logits) if c > s
    )
    semicolon_preferred = sum(
        1 for c, s in zip(all_comma_logits, all_semicolon_logits) if s > c
    )
    ...
    logger.info(
        ...
        "comma_preferred=%d/%d, semicolon_preferred=%d/%d",
        mean_comma_logit,
        mean_comma_prob,
        mean_semicolon_logit,
        mean_semicolon_prob,
